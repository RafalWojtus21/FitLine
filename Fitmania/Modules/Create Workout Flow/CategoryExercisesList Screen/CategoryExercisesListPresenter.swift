//
//  CategoryExercisesListPresenter.swift
//  Fitmania
//
...
final class CategoryExercisesListPresenterImpl: CategoryExercisesListPresenter {
    typealias View = CategoryExercisesListView
    typealias ViewState = CategoryExercisesListViewState
    typealias Middleware = CategoryExercisesListMiddleware
    typealias Interactor = CategoryExercisesListInteractor
    ...
    func bindIntents(view: View, triggerEffect: PublishSubject<Effect>) -> Observable<ViewState> {
        let intentResults = view.intents.flatMap { [unowned self] intent -> Observable<Result> in
            switch intent {
            case .viewLoaded:
                return interactor.loadExercises()
            case .cellTapped(chosenExercise: let chosenExercise):
                return .just(.effect(.showAddExerciseScreen(exercise: chosenExercise)))
            }
        }
